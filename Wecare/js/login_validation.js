...
function login(event){
        ...
        if ((password.length <= 7) || (!nums.test(password)) || (!schar.test(password)) || (password.length >= 16)||(!schar2.test(password))) {
        ...
        if (!nums.test(username) || !letters.test(username) || schar.test(username) || (!letters.test(username.charAt(0)))) {
...
function validate(event) {
    ...
    if (error != 0) {
        return false;
           ...
           alert("success!! "+"First name: " + firstname + "\nLast name: " + lastname + "\nEmail: " + email + "\nPassword: " + password);
